...
| 2026-06-10 | Language-mirroring (ADR-008) реализован и **подтверждён на проде** (`https://velunoapp.shop`): EN-вопрос по RU-контексту → EN-ответ; RU→RU; DE→DE. Тесты 146/146 зелёные. Architect: синхронизировал docs с фактической реализацией — сигнатура `detect_response_language(message, transcription_language)` (2 аргумента, fallback на язык транскрибации) в ADR-008 §1 и 03-architecture §1; зафиксировано финальное v1-поведение script-guard (кириллица + латиница; иная письменность → fallback, а не ISO-код по script'у). |
